geolibs <- c("spdep","ggmap","rgdal","rgeos","maptools","dplyr","tidyr","tmap","raster", "dplyr", "tidyr","assertthat","data.table")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~
#REPEAT FOR COUNTRY OF BIRTH 1991: SAS VERSION----
CoB91 <- read.csv("VariableCoding/CountryOfBirth_fiveCensusRecodes/91SAS_PCS.csv")
...
#aggregate by zone for all columns
#http://stackoverflow.com/questions/21295936/can-dplyr-summarise-over-several-variables-without-listing-each-one
agg2 <- CoB91[,2:16] %>% group_by(aggID) %>% 
  summarise_each(funs(sum))
...
#Reformat to match other CoB saves so they can all be rbinded elsewhere


#Write this one to the 5 census PCS folder instead
writeSpatialShape(geogz,
