...
geolibs <- c("spdep","ggmap","rgdal","rgeos","maptools","dplyr","tidyr","tmap","dplyr", "tidyr","assertthat","data.table",'readr','pryr','combinat','purrr','forcats')
...
#UPDATE FOR USING 100% XIJ AT COUNTRY LEVEL
#Just subset the main sheet
sheet5 <- readRDS('R_data/sheet5.rds')

#And subset appropriately.
#Check each CoB is summing to 100... Tick
sheet5 %>% group_by(cob) %>%
  summarise(sum(xij1971))

#And there are 822 zones... tick
unique(sheet5$label) %>% length
...
#Now: just need to have one set with just rest-of-world removed
#And another with 2 rich 2 poor cobs
sheet5allbutRoW <- sheet5[sheet5$cob!='Rest_of_Wo',]
sheet5tworichpoor <- sheet5[sheet5$cob %in% c('Pakistan','India','Old_Common','Other_Euro'),]

#cities for both need subsetting into lists
citiesList <- c('Glasgow','Edinburgh','Aberdeen','Dundee')

#This puts in alphabetical order
citiesNoRow <- split(sheet5allbutRoW[sheet5allbutRoW$name %in% citiesList,],
                     sheet5allbutRoW[sheet5allbutRoW$name %in% citiesList,]$name)

#Just in case it matters for anything later, keeping original order
citiesNoRow <- citiesNoRow[c(4,3,1,2)]

citiesTworichpoor <- split(sheet5tworichpoor[sheet5tworichpoor$name %in% citiesList,],
                           sheet5tworichpoor[sheet5tworichpoor$name %in% citiesList,]$name)

#Just in case it matters for anything later, keeping original order
citiesTworichpoor <- citiesTworichpoor[c(4,3,1,2)]

saveRDS(sheet5allbutRoW,'R_data/sheet5AllScotsAllbutROW.rds')
saveRDS(citiesNoRow,'R_data/citySheets5AllScotsAllbutROW.rds')

saveRDS(sheet5tworichpoor,'R_data/sheet5AllScotstworichpoor.rds')
saveRDS(citiesTworichpoor,'R_data/citySheets5AllScotstworichpoor.rds')

#check they look right
plot(sheet5allbutRoW$xij2011[sheet5allbutRoW$cob == "Pakistan"],sheet5allbutRoW$xij2001[sheet5allbutRoW$cob == "Pakistan"])
...
#UPDATE FOR STICKING TO 100% XIJ SUM FOR WHOLE OF SCOTLAND
fiveCensusPullOutCoeffs <- function(year,filenameadd){
  ...
  if(filenameadd=='allbutRoW'){
    ...
    sheet5 <- readRDS('R_data/sheet5AllScotsAllbutROW.rds')
    citySheet5 <- readRDS('R_data/citySheets5AllScotsAllbutROW.rds')
    ...
    sheet5 <- readRDS('R_data/sheet5AllScotstworichpoor.rds')
    citySheet5 <- readRDS('R_data/citySheets5AllScotstworichpoor.rds')
  ...
  write.csv(allrez,paste0('regressionOutputs/allScot4xijshare/5census/5censusTableCombo',filenameadd,'_',year,'.csv'), row.names = F)
  ...
  #save
  write.csv(allrezwide,paste0('regressionOutputs/allScot4xijshare/5census/wide',filenameadd,'_',year,'.csv'), row.names = F)
...
#CITIES 3 CENSUS
#UPDATE: run only on subset of all-scotland data not each city xij summing to 100
for(i in 1:4){
  
  print(citiesList[[i]])
  
  rez <- lapply(split(sheet[sheet$name == citiesList[[i]] & sheet$urbanFiftyPercentPlus==1,],
                      sheet[sheet$name == citiesList[[i]] & sheet$urbanFiftyPercentPlus==1,][,'rich']), 
                function(x) summary(lm(data=x, formula = formula3census)))
...
#save
write.csv(allrez3,paste0('regressionOutputs/allScot4xijshare/3census/3censusTableCombo_1991.csv'), row.names = F)
...
#save
write.csv(allrez3wide,paste0('regressionOutputs/allScot4xijshare/3census/wide3censusTableCombo_1991.csv'), row.names = F)
...
#Reload
rez3census <- read_csv('regressionOutputs/allScot4xijshare/3census/3censusTableCombo_1991.csv')

rez5census71 <- read_csv('regressionOutputs/allScot4xijshare/5census/5censusTableComboallbutRoW_1971.csv')
rez5census91 <- read_csv('regressionOutputs/allScot4xijshare/5census/5censusTableComboallbutRoW_1991.csv')
rez5census71fourcobs <- read_csv('regressionOutputs/allScot4xijshare/5census/5censusTableCombo4cobs_1971.csv')
rez5census91fourcobs <- read_csv('regressionOutputs/allScot4xijshare/5census/5censusTableCombo4cobs_1991.csv')
...
allz2$source <- '3 census: 2011 ~ 1991'
allz2$source[grepl('5census',allz2$oldsource,ignore.case = T)
             & grepl('1991',allz2$coeff,ignore.case = T)] <- '5 census: 2011 ~ 1991'
allz2$source[grepl('5census',allz2$oldsource,ignore.case = T)
             & grepl('1971',allz2$coeff,ignore.case = T)] <- '5 census: 2011 ~ 1971'
...
allz2$coeff <- 'Spatial persistence'
allz2$coeff[grepl('q',allz2$oldcoeff)] <- 'Homophily horizon'
allz2$coeff[grepl('migshare',allz2$oldcoeff)] <- 'Diversity aversion'
allz2$coeff[grepl('lph',allz2$oldcoeff)] <- 'House price (log)'
allz2$coeff[grepl('hsperacre',allz2$oldcoeff)] <- 'Density'
allz2$coeff[grepl('lpophs',allz2$oldcoeff)] <- 'Crowding'
allz2$coeff[grepl('ea',allz2$oldcoeff)] <- 'Percent employed'
...
allz2$source <- factor(allz2$source, levels = c(
  "3 census: 2011 ~ 1991","5 census: 2011 ~ 1971","5 census: 2011 ~ 1991", '5 census ~ 1991, 2 rich 2 poor', '5 census ~ 1971, 2 rich 2 poor'
))
...
#     
#     #Annoying! If there's only one value it changes ... 
#     #Mean will be 1 if they're all significant
...
#   
#   ggsave(paste0('R_outputs/coefficientFacetPlots/AllCoefficents_Oct17xij100allScotland_freeAxis/facet_',coefftouse,'.png'), 
...
#Remove 2 rich 2 poor
allz2NoAberDundee3cats <- allz2NoAberDundee %>% filter(!grepl('rich',source,ignore.case = T))

#Change coefficient names to something more publishy



#Factor up rich/poor order. Watch out for issues.
allz2NoAberDundee3cats$rich <- factor(allz2NoAberDundee3cats$rich, levels = c('rich','poor'))

#Label significance
allz2NoAberDundee3cats$sig[allz2NoAberDundee3cats$sig==0] <- 'no'
allz2NoAberDundee3cats$sig[allz2NoAberDundee3cats$sig==1] <- 'yes'

allz2NoAberDundee3cats$sig <- factor(allz2NoAberDundee3cats$sig,levels = c('no','yes'))

for(coefftouse in unique(allz2NoAberDundee3cats$coeff)){
  
  subz <- allz2NoAberDundee3cats %>% filter(coeff==coefftouse)
  
  #To make width sensible with varying source number for different coeffs
  numsources <- unique(subz$source) %>% length
  print(paste0('Number of sources: ', numsources))
  
  #cos sig factored, 2 is 'yes'
  if(mean(as.numeric(subz$sig))==2) {
    sigpoints <- c(16,4)} else {
      sigpoints <- c(4,16)}
  
  output <- ggplot(subz, aes(x = zones, y = Estimate, colour = rich, shape = factor(sig))) +
    geom_point(size = 5, position = position_dodge(width = -0.5)) +
    geom_errorbar(aes(ymin = minn, ymax = maxx),width = 0.5, position = position_dodge(width = -0.5)) +
    coord_flip() +
    # scale_shape_manual(values=c(4,16),breaks = c("0", "1")) +
    ...
    facet_wrap(~source, nrow = 1, scales = 'free_x') +
    ggtitle(paste0(coefftouse)) +
    theme(plot.title = element_text(face="bold",hjust = 0.5),
          strip.text = element_text(face = "bold")) +
    guides(shape = guide_legend(title='p < 0.05')) +
    geom_hline(yintercept = 0, 
               alpha = ifelse(min(subz$minn) < 0, 0.3, 0))#only show if error bars cross zero
    
  
  ggsave(paste0('R_outputs/coefficientFacetPlots/AllCoefficents_Oct17xij100allScotland_prettify/facet_',coefftouse,'.png'), 
         dpi = 300, height = 4, width = 2.5 + (numsources * 2.5))
